...
public class SLEReport extends Report {
	...
	/**
	 * read a CSV file and create the excel vertion of it.
	 * 
	 * @param tf - temporal file in csv format
	 * @param ef - destination excel file private static void excelExport(File tf, String ef) { Iterable<CSVRecord>
	 *        inrlst = null; try { Reader in = new FileReader(tf); // Reader in = new
	 *        FileReader("C:/Users/terry/Desktop/a.csv"); inrlst = (new CSVParser(in, CSVFormat.EXCEL).getRecords());
	 * 
	 *        // create excel Workbook wb = ef.endsWith(".xls") ? new HSSFWorkbook() : new XSSFWorkbook(); Sheet sheet =
	 *        wb.createSheet("InterfaceJDE"); int rowcnt = 0, colcnt = 1; String acol = null, bcol = null; for
	 *        (CSVRecord csvr : inrlst) { Row row = sheet.createRow(rowcnt++); for (int c = 0; c < csvr.size(); c++) {
	 *        String sval = csvr.get(c); colcnt = colcnt < c ? c : colcnt;
	 * 
	 *        // values for a and b cols are last value diferent of "" acol = (c == 0 && !sval.equals("")) ? sval :
	 *        acol; bcol = (c == 1 && !sval.equals("")) ? sval : bcol; if (c == 0) sval = new String(acol); if (c == 1)
	 *        sval = new String(bcol);
	 * 
	 *        Cell cell = row.createCell(c); cell.setCellValue(sval); if (c > 3) { sval = sval.replaceAll("[.]", "");
	 *        sval = sval.replaceAll("[,]", "."); try { double d = Double.parseDouble(sval); CellStyle style =
	 *        wb.createCellStyle(); style.setAlignment(HorizontalAlignment.RIGHT);
	 *        style.setDataFormat(wb.createDataFormat().getFormat("#,##0.00")); cell.setCellStyle(style);
	 *        cell.setCellValue(d); } catch (Exception e) {
	 * 
	 *        } } } }
	 * 
	 *        // column size for (int co = 0; co < colcnt + 1; co++) { sheet.autoSizeColumn(co); }
	 * 
	 *        // Write the output to a file FileOutputStream out = new FileOutputStream(ef); wb.write(out); out.close();
