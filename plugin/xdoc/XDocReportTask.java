...
public class XDocReportTask implements TRunnable {
	...
	private void generateXDcox(Record baseRcd) {
		...
		try {
			...
			Record docrcd = ConnectionManager.getAccessTo(PluginManager.getMyFile("XDoc", "xd_config")).exist(
					"xd_coid = '" + docid + "'");
			...
			// to mergedocs
			String mdf = PluginManager.getMyFile("XDoc", "xd_merged_doc");
			Record mr = ConnectionManager.getAccessTo(mdf).getModel();
			...
			ConnectionManager.getAccessTo(mdf).add(mr);
